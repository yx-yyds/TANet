...
def u2net_full(out_ch: int = 1):
    cfg = {
        # height, in_ch, mid_ch, out_ch, RSU4F, side(是否需要side输出)
...
if __name__ == '__main__':
    ...
    u2net = u2net_full()
    print("模型参数为：{:.2f}M".format(sum(p.numel() for p in u2net.decode_modules.parameters() if p.requires_grad)/1024/1024))
    img = torch.rand([14, 3, 256, 256])
    output = u2net(img)
